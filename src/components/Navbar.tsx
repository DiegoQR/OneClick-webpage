...
export default function Navbar() {
  ...
  useGSAP((_, contextSafe) => {
      ...
      const hoverInAnimation = contextSafe!((e : HTMLElement) => {
        gsap.set(e.getElementsByClassName('animated-box'), {scaleY: 0, backgroundColor: '#FE7743', transformOrigin: 'bottom'});
        gsap.to(e.getElementsByClassName('animated-box'), 
        { 
          scaleY: 1, 
          duration: 0.5, 
          ease: 'bounce.out',
    ...
    <nav className='flex justify-between items-stretch px-20 bg-blue'>
      ...
      <ul className='flex items-stretch font-bold text-sm text-white'>
        <li className='hover-target flex items-center px-10 relative cursor-pointer'>
            ...
            <div className='animated-box w-full h-full absolute inset-x-0 bottom-0'></div>
        </li>
        <li className='hover-target flex items-center px-10 relative cursor-pointer'>
            <div className='z-1 inline-flex items-center gap-2'>
